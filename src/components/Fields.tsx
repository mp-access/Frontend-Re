    ...
    .when("added", {
      ...
      then: (s) =>
        s.oneOf(
          ...
          "set Use Context for all selecteded file",
        ),
    }),
})
...
const contactSchema = memberSchema.concat(
  yup.object({
    message: yup.string().ensure().max(500).min(5),
    topic: yup.string(),
  }),
)
...
const courseSchema = yup.object({
  ...
  slug: yup
    ...
    .ensure()
    .trim()
    .matches(/[0-9a-z]/, "Lowercase letters, numbers or dash (-) only"),
  ...
  logo: yup
    ...
    .test(
      "image",
      (p) => (p?.value ? "Invalid" : "Required"),
      (a) => a?.startsWith("data:image"),
    ),
})
const assignmentSchema = courseSchema
  .pick(["title", "slug", "startDate", "endDate", "description"])
  .concat(
    yup.object({ ordinalNum: yup.number().min(0).nullable().default(null) }),
  )
const taskSchema = assignmentSchema
  .pick(["title", "slug", "ordinalNum"])
  .concat(
    yup.object({
      ...
      files: yup
        ...
        .test(
          ...
          (params) => !!params?.find((f) => f.added),
        )
        .test(
          "hasTask",
          'Add at least 1 editable file with "Task" context',
          (params) =>
            !!params?.find(
              (f) => f.added && f.editable && f.context === "Task",
            ),
        )
        .test(
          "hasInst",
          'Add exactly one file with "Instructions" context',
          (params) =>
            params?.filter((f) => f.added && f.context === "Instructions")
              .length === 1,
        ),
    }),
  )
...
export const FormField = ({
  ...
  return (
    <Controller
      name={name || camel(title)}
      control={control}
      render={({ field, fieldState }) => (
        <FormControl isInvalid={!!fieldState.error}>
          <HStack w="full" overflow="hidden" align="stretch">
            <FormLabel textTransform="capitalize" whiteSpace="nowrap">
              {title || name}
            </FormLabel>
            <Flex pos="relative" flexGrow={1}>
              <FormErrorMessage pos="absolute" right={1} top={-1}>
        ...
        <FormControl isInvalid={!!fieldState.error}>
          {selectOptions[placeholder] ? (
            <Select
              ...
              placeholder="Select"
            >
              {selectOptions[placeholder].map((o) => (
            ...
            {range(fields.length).map((i) => (
              <Tr key={i}>
                ...
                {columns.map((column) => (
                  <Td key={column} p={0}>
                    <ColumnField
                      name={`${name}.${i}.${camel(column)}`}
                      placeholder={column}
                    />
                  </Td>
                ))}
                <Td w={3} p={0}>
                  <CloseButton ml={1} size="sm" onClick={() => remove(i)} />
                </Td>
              </Tr>
            ))}
            <Tr></Tr>
