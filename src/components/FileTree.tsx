...
export function FileTree({ files, selected, onSelect }: RootNodeProps) {
  ...
  const onClick = (item: TreeItem) =>
    setExpandedItems((items) => toggle(items, item.index))

  return (
    <ControlledTreeEnvironment
      items={tree}
      getItemTitle={(item) => item.data.name}
      renderDepthOffset={13}
      children={
        <Tree treeId="task" rootItem={head(split(files[0].path, "/")) || ""} />
      }
      viewState={{
        ["task"]: {
      ...
      onExpandItem={onClick}
      onCollapseItem={onClick}
      renderItemTitle={({ item, context }) => (
        <HStack
          boxSize="full"
          onClick={() => !item.isFolder && onSelect(item.data)}
          color="gray.600"
          pl={2}
        >
          <NodeIcon
            name={
              (item.isFolder && `folder-${!!context.isExpanded}`) ||
              detectType(item.data.name)
            }
            boxSize={4}
          />
          <Text fontFamily="file" fontSize="sm">
            {item.data.name}
          </Text>
        </HStack>
      )}
