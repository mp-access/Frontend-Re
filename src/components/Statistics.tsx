  ...
  return (
    <ResponsiveContainer>
      <PieChart height={120} width={100}>
        <Pie
          data={[{ current }, { current: max - current }]}
          dataKey="current"
          innerRadius="80%"
          outerRadius="100%"
          startAngle={90}
  ...
  <ResponsiveContainer>
    <PieChart>
      ...
      >
        ...
        <Cell key="cell-1" opacity={0.15} />
        <Label
          value={`${formatPoints(value)} / ${formatPoints(max)}`}
          color="inherit"
          fill="currentColor"
          dy={-5}
          position="center"
  ...
  return (
    <Stack align="end" justify="end" spacing={0}>
      <Text
        ...
        px={2}
        fontSize="sm"
      >{`${t("Score")} ${isNull(value) ? "?" : formatPoints(value)} / ${formatPoints(max)}`}</Text>
